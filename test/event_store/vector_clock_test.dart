...
void main() {
  group('EventClock', () {
    ...
    test('update event from a known device', () {
      final clock = EventVectorClock({deviceId1: timestamp1});
    ...
    test('update event that is behind the latest known event', () {
      final clock = EventVectorClock({deviceId1: timestamp2});
      expect(
    ...
    test('compare two EventClocks with the same deviceTimestamp map', () {
      final clock1 = EventVectorClock({
        deviceId1: timestamp1,
        deviceId2: timestamp2,
      });
    ...
    test('compare two EventClocks with a different number of devices', () {
      ...
      final clock2 = EventVectorClock({deviceId1: timestamp1});
      expect(clock1.compareTo(clock2), equals(1));
    });

    test('compare two EventClocks with different timestamps', () {
      final clock1 = EventVectorClock({deviceId1: timestamp1});
      final clock2 = EventVectorClock({deviceId1: timestamp2});
      expect(clock1.compareTo(clock2), equals(-1));
    });
  });
}
